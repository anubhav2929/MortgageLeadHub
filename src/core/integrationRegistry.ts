...
export const INTEGRATIONS: IntegrationDef[] = [
  {
    ...
    requiredKeys: ["TELNYX_API_KEY", "TELNYX_PHONE_NUMBER", "TELNYX_PUBLIC_KEY"],
    fields: [
      ...
      { key: "TELNYX_MESSAGING_PROFILE_ID", label: "Messaging profile ID", secret: false, optional: true, help: "Recommended: paste the profile assigned to the sending number so the CRM explicitly uses its 10DLC campaign and webhooks." },
      { key: "TELNYX_PUBLIC_KEY", label: "Webhook Ed25519 public key", secret: false, help: "Portal → Account Settings → Public Key. Required for signed inbound and delivery webhooks." },
      {
    ...
    setupSteps: [
      ...
      "Buy an SMS-capable US local number, create a messaging profile, and assign the number to that profile.",
      "Create a least-privilege API key under Auth → API Keys and paste it above.",
      "Register the company brand and a direct-lending 10DLC campaign using the exact intake disclosure, message flow, sample templates, privacy/terms URLs, sender identity, and STOP handling. Wait until the campaign is active, then assign it to the messaging profile.",
      "Copy Account Settings → Public Key above. On the messaging profile set API v2 POST primary webhook to /api/webhooks/telnyx and failover webhook to /api/webhooks/telnyx/failover. Both endpoints validate Ed25519 signatures and replay age.",
      "Enable Advanced Opt-In/Out on the messaging profile. Telnyx sends the configured STOP/START/HELP response; the CRM records suppression without sending a duplicate response.",
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "RESEND_REPLY_TO_EMAIL", label: "Inbound Reply-To base", secret: false, optional: true, placeholder: "replies@reply.yourdomain.com", help: "Use an address on your Resend receiving domain. The CRM adds a per-lead +alias so replies attach to the exact inquiry." },
      {
    ...
    setupSteps: [
      "Create an account at resend.com, add the company sending domain, and publish/verify its SPF and DKIM DNS records.",
      "Create a least-privilege sending API key, paste it above, and set From address to a mailbox on the verified domain.",
      "For delivery tracking: subscribe email.sent, email.delivered, email.delivery_delayed, email.bounced, email.complained, email.failed, and email.suppressed at /api/webhooks/delivery/resend, then paste its signing secret above.",
      "For inbound replies: add a separate receiving domain (a subdomain such as reply.example.com avoids changing the main mail MX), publish its Resend MX record, and add an email.received webhook pointing at /api/webhooks/resend-inbound. Paste that webhook's own signing secret above.",
      "Set Inbound Reply-To base to an address on that receiving domain. Outbound lead messages use a per-lead +publicRef alias; the CRM also verifies the reply sender before attaching content to the lead.",
    ],
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "VAPI_ASSISTANT_NAME", label: "Assistant caller name", secret: false, optional: true, placeholder: "Anna", help: "Spoken in the personalized opening before Equity Flow Group. Defaults to Anna." },
      { key: "VAPI_VOICE_PROVIDER", label: "Voice provider", secret: false, optional: true, placeholder: "vapi", help: "Use vapi for built-in voices or the exact Vapi provider ID, such as 11labs, for a connected provider." },
      { key: "VAPI_VOICE_ID", label: "Voice ID", secret: false, optional: true, placeholder: "Savannah", help: "Copy the exact voice ID from Vapi. Defaults to the Vapi-built-in Savannah voice." },
      { key: "VAPI_VOICE_MODEL", label: "Voice model", secret: false, optional: true, placeholder: "eleven_v3", help: "Only set when the selected voice provider requires a model." },
      { key: "VAPI_TRANSCRIBER_PROVIDER", label: "Transcriber provider", secret: false, optional: true, placeholder: "soniox", help: "Leave both transcriber fields blank to use Vapi's managed default." },
      { key: "VAPI_TRANSCRIBER_MODEL", label: "Transcriber model", secret: false, optional: true, placeholder: "stt-rt-v5", help: "For the screenshot Soniox setup, use provider soniox and model stt-rt-v5." },
      ...
      { key: "VAPI_WEBHOOK_SECRET", label: "Webhook authentication token", secret: true, help: "Generate a random token. Use the same value in the Vapi Bearer Custom Credential, or as server.secret for an existing assistant." },
      { key: "VAPI_WEBHOOK_CREDENTIAL_ID", label: "Webhook credential ID", secret: false, optional: true, help: "Recommended: a Vapi Custom Credential with Authorization header, Bearer prefix, and this app's webhook token as its value." },
    ...
    setupSteps: [
      ...
      "Generate a random webhook token. In Vapi, create a Custom Credential that sends Authorization: Bearer <that token>, then paste its credential ID here. Existing server.secret assistants remain supported through X-Vapi-Secret.",
      "The CRM creates a transient assistant or squad for each outbound call. Do not assign a separate static qualification prompt to the outbound number; it can diverge from the server sequence.",
      "Set Assistant caller name to the name the voice should introduce, such as Anna. Each outbound call automatically receives the lead's validated first name, last name, city, intake snapshot, and approved cross-channel context.",
      "To mirror a tested Vapi pipeline, copy its exact voice provider, voice ID/model, model provider/model, and transcriber provider/model into these fields. Leave transcriber blank to use Vapi's managed default.",
  ...
  },
  {
    ...
    setupSteps: [
      "APP_URL is optional on Vercel. When blank, the CRM uses VERCEL_PROJECT_PRODUCTION_URL (the stable connected production domain), then the generated deployment URL as a fallback.",
