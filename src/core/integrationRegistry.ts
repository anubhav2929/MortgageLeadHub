...
export const INTEGRATIONS: IntegrationDef[] = [
  ...
  },
  {
    ...
    name: "Vapi",
    category: "Voice AI",
    powers: "Live calls through one published Vapi assistant; the CRM supplies only lead context and correlation IDs.",
    requiredKeys: ["VAPI_API_KEY", "VAPI_PHONE_NUMBER_ID", "VAPI_ASSISTANT_ID", "VAPI_WEBHOOK_SECRET"],
    fields: [
      { key: "VAPI_API_KEY", label: "Private API key", secret: true },
      { key: "VAPI_PHONE_NUMBER_ID", label: "Phone number ID", secret: false, help: "The saved Vapi phone-number UUID, not its E.164 number." },
      { key: "VAPI_ASSISTANT_ID", label: "Published assistant ID", secret: false, help: "The UUID of the tested assistant whose voice, model, prompt, tools, and server settings live in Vapi." },
      { key: "VAPI_WEBHOOK_SECRET", label: "Webhook authentication token", secret: true, help: "Use this exact token in the Bearer Custom Credential attached to the assistant and all CRM tools in Vapi." },
