...
export const capabilities = {
  ...
  hasLiveVoiceAgent: Boolean(env.VAPI_API_KEY && env.VAPI_PHONE_NUMBER_ID && env.VAPI_ASSISTANT_ID && env.VAPI_WEBHOOK_SECRET),
...
export function announceCapabilitiesOnce() {
  ...
  const lines = [
    ...
    `Voice AI agent (Vapi): ${capabilities.hasLiveVoiceAgent ? "LIVE — saved assistant + outbound number + webhook wired" : capabilities.hasVoiceAgent ? "Vapi key present but VAPI_PHONE_NUMBER_ID/VAPI_ASSISTANT_ID/VAPI_WEBHOOK_SECRET missing — see adapters/voiceAgent.ts" : "not configured — set VAPI_API_KEY, VAPI_PHONE_NUMBER_ID, VAPI_ASSISTANT_ID, VAPI_WEBHOOK_SECRET"}`,
